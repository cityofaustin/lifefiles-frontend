  ...
  static deleteOwnerAccount() {
    ...
    window.location.replace(
      AccountService.getAuthApi() +
        `/unregister?access_token=${jwt}&client_id=${process.env.REACT_APP_CLIENT_ID}&response_type=code&redirect_uri=${location.origin}${location.pathname}&scope=${scope}&state=${state}`
    );
  }
}
